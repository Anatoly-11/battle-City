...
Bullet::Bullet(const double velocity, const glm::vec2& position, const glm::vec2& size, const glm::vec2& explosionSize,
  const float layer) noexcept : IGameObject(IGameObject::EObjectType::Bullet, position, size, 0.f, layer),
...
void Bullet::render() const noexcept {
  if(m_isActive) {
    if(m_isExplosion) {
...
void Bullet::update(const double delta) noexcept {
  if(m_isExplosion) {
...
void Bullet::fire(const glm::vec2 &position, const glm::vec2 &direction) noexcept {
  m_position = position;
...
void Bullet::onCollision() noexcept {
  setVelocity(0);
  m_isExplosion = true;
